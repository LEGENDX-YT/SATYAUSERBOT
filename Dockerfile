...
RUN wget -O opencv.zip https://github.com/opencv/opencv/archive/Satya.zip && unzip opencv.zip && mv -f opencv-Satya /usr/bin/ && rm opencv.zip
RUN git clone https://github.com/SATYA-OP/SATYAUSERBOT /root/userbot
RUN mkdir /root/userbot/bin/
WORKDIR /root/userbot/
RUN chmod +x /usr/local/bin/*
RUN pip3 install -r requirements.txt
CMD ["bash","./SATYAUSERBOT/start.sh"]
